...
// 将极坐标表示的点(r, phi) 转换为笛卡尔坐标表示的点(pred_xs, pred_ys)
...
)
{
    ...
    if (pred_cls.sizes()[0] != 1)
    {
        ...
        rphi_to_xy(pred_r, pred_phi, pred_xs, pred_ys);//坐标系转换
        ...
        for (int i = 0; i < num_pts; i++)
        {
            if (!keep[i].item<bool>())
                continue;
            torch::Tensor dup_inds = p_dist[i].lt(min_dist);

            torch::Tensor updated_instance_mask = torch::zeros_like(instance_mask);
            auto aaa = sorted_indices[{0}].index({ dup_inds });
           for (auto i = 0;i< aaa.sizes()[0];i++)
           {
               updated_instance_mask[i] = 1;
           }

            instance_mask.index_put_({ updated_instance_mask.to(torch::kBool) }, instance_id);
            keep.index_put_({ dup_inds.to(torch::kBool)}, 0);
            keep.index_put_({ i }, 1);
            instance_id += 1;
        }
        ...
        dets_xys = stacked.index({ keep.nonzero().squeeze(1) });
        dets_cls = pred_cls.index({ "...",0 }).masked_select(keep);
